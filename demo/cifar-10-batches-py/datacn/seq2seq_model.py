...
import numpy as np
from six.moves import xrange  ##生成随机数
...
class Seq2SeqModel(object):
  ...
  def __init__(self,
    ...
    # 当采样数小于vocabulary size 时 Sampled softmax 才有意义.
    if num_samples > 0 and num_samples < self.target_vocab_size:
      w_t = tf.get_variable("proj_w", [self.target_vocab_size, size], dtype=dtype)
      w = tf.transpose(w_t)
      b = tf.get_variable("proj_b", [self.target_vocab_size], dtype=dtype)
      output_projection = (w, b)
      #自定义LOSS函数，计算交叉熵，后面使用model_with_buckets时，作为参数传入
    ...
    # 使用词向量作为输入，定义了整体网络架构，后面使用model_with_buckets时，作为参数传入.
...
#      cell = tf.contrib.rnn.MultiRNNCell(cells)
      ...
      return tf.contrib.legacy_seq2seq.embedding_attention_seq2seq(
          encoder_inputs,#输入层编码，一个形状为[batch_size]的list
          decoder_inputs,#解码层输入，一个形状为[batch_size]的list
          cell,#定义的多层cell结构
          num_encoder_symbols=source_vocab_size,#源数据对应词总个数
          num_decoder_symbols=target_vocab_size,#目标数据对应词总个数
          embedding_size=size,#每个输入对应的词向量大小
          output_projection=output_projection,#对输出结果是否进行全连接维度转化，如果需要转化，则传入全连接对应的W和B
          feed_previous=do_decode,#为True时表明只有第一个Decoder输入以GO开始，其他都使用前面的状态，如果为FALSE ,每个Decoder的输入都会以GO开始。GO为自已定义模型时定义的一个起始符，一般用0或1来指定
          dtype=dtype)

    #  生成需要的编码器词汇LIST，解码器词汇LIST，标签词汇LIST（targets）
    self.encoder_inputs = []
    self.decoder_inputs = []
    self.target_weights = []
    ...
    tf.contrib.legacy_seq2seq.model_with_buckets
        ...
        decoder_inputs
        targets#输出结果的LABEL
        weights:传入的权重值，必须与decoder_input的SIZE相同
        buckets传入的桶，描述为[（XX,XX）,(XX,XX))],每一对有两个数，第一个数为输入SIZE,第二个为输出SIZE
        seq2seq:带有attention机制的Seq2Seq网络，以函数名传入
        softmax_loss_function：是否使用自定义的Loss函数
        per_example__loss:是否对每个样本求Loss    
    """
    # 如果是测试.
    if forward_only:
    ...
    else:#如果是训练，使用contrib.legacy_seq2seq.model_with_buckets将Seq2Seq网络，自定义loss函数
      self.outputs, self.losses = tf.contrib.legacy_seq2seq.model_with_buckets(
          self.encoder_inputs, self.decoder_inputs, targets,
    ...
    if not forward_only:#如果不是测试
      self.gradient_norms = []
      self.updates = []
      opt = tf.train.GradientDescentOptimizer(self.learning_rate)
      for b in xrange(len(buckets)):
        gradients = tf.gradients(self.losses[b], params)#计算LOSS对应参数params的梯度
        clipped_gradients, norm = tf.clip_by_global_norm(gradients,
                                                         max_gradient_norm)#将过大的梯度按照max_gradient_norm来截断
  ...
  def step(self, session, encoder_inputs, decoder_inputs, target_weights,
    ...
    if len(encoder_inputs) != encoder_size:
      raise ValueError("编码器长度必须等于桶中的一个,"
                       " %d != %d." % (len(encoder_inputs), encoder_size))
    if len(decoder_inputs) != decoder_size:
      raise ValueError("解码器长度必须等于桶中的一个,"
                       " %d != %d." % (len(decoder_inputs), decoder_size))
    if len(target_weights) != decoder_size:
      raise ValueError("Weights长度必须等于桶中的一个,"
    ...
    #如果是测试outputs[0]为LOSS, outputs[1:] 为decoder_size序列
    if not forward_only:
      return outputs[1], outputs[2], None  # 更新的梯度, loss, no outputs.
    else:
