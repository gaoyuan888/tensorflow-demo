...
def get_ch_lable_v(txt_file,word_num_map,txt_label=None):
    ...
    #在字表中找到相应字的索引，如无则使用words_size代替  
...
n_hidden2 = 512
n_hidden3 = 512
# 定义传入参数，X代表输入连续3个文字，wordy则代表一个字
...
# Loss optimizer
loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=pred, labels=wordy))
...
with tf.Session() as session:
    ...
    while step < training_iters:
        ...
        # 随机取一个位置偏移
        #如果已经训练到文件最后几个文字，偏移量已经超过了文件尾，就重置OFFSET
        if offset > (len(training_data)-end_offset):
            offset = random.randint(0, n_input+1)
       #按照指定的位置偏移获取后面的3个文字向量当作输入
        ...
        #设置标签值，即inwords之后的字向量
        out_onehot[wordlabel[offset+n_input]] = 1.0
        #print("out_onehotwordlabel",out_onehot)
        #reshape成[1，68]的onehot形式
        out_onehot = np.reshape(out_onehot,[1,-1])
        #print("out_onehot",out_onehot)
    ...
    while True:
        ...
        if len(inputword) != n_input:
            print("您输入的字符长度为：",len(inputword),"请输入3个字")
            continue
        try:
