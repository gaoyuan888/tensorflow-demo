...
import numpy as np
import math#算术库
import collections#计算词频用
import pickle as pkl#保存、读取楧型时用
import jieba#pip install jieba中文分词时用
import os#取系统路径
...
class word2vec():
    def __init__(self,
        ...
        # 获得模型的基本参数
        self.batch_size     = None # 一批中有多少数据, 目前是根据情况来的
        if model_path!=None:#如果已存在模型数据
            self.load_model(model_path)
        else:
            # model parameters
            assert type(vocab_list)==list#判断是否是LIST类型
            ...
            # train times
            self.train_words_num = 0 # 已训练的单词数
            self.train_sents_num = 0 # 已训练的句子数
            self.train_times_num = 0 # 已训练的次数（一次可以有多个句子）

            # train loss records
            self.train_loss_records = collections.deque(maxlen=10) # 保存最近10次的误差
            self.train_loss_k10 = 0#十次误差均值
    ...
    def build_graph(self):
        ...
        with self.graph.as_default():
            ...
            # 得到NCE损失,具体参考https://www.jianshu.com/p/fab82fa53e16
            ...
            for i in range(sent.__len__()):
                ...
                for index in range(start,end):
                    if index == i:
                        ...
                        input_id = self.word2id.get(sent[i])#生成input
                        label_id = self.word2id.get(sent[index])#生成label
...
if __name__=='__main__':
    ...
    with open('stop_words.txt',encoding= 'utf-8') as f:
        line = f.readline()
        while line:
            stop_words.append(line[:-1])#去换行符
            line = f.readline()
    stop_words = set(stop_words)#去重复词
    ...
    with open('2800.txt',encoding='gbk') as f:
        ...
        while line:
            while '\n' in line:
                line = line.replace('\n','')#替换行符为''，数据清洗
            while ' ' in line:
                line = line.replace(' ','')#替空格为''，数据清洗
            if len(line)>0: # 如果句子非空
                raw_words = list(jieba.cut(line,cut_all=False))#将一句话分词
                dealed_words = []
                for word in raw_words:
                    if word not in stop_words and word not in ['qingkan520','www','com','http']:
                        raw_word_list.append(word)
                        dealed_words.append(word)#分好词的句子
                sentence_list.append(dealed_words)#加入分好词的句子
            line = f.readline()
    word_count = collections.Counter(raw_word_list)
    print('文本中总共有{n1}个单词,不重复词数{n2},选取前30000个单词进入词典'
